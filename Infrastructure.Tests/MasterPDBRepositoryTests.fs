...
let ``Put and get master PDB`` () =
    let pdbName = "test1"
    let repo = MasterPDBRepository(onRepoFailure, testFolder, pdbName) :> IMasterPDBRepository
...
let ``Save and load master PDB`` () =
    ...
    let pdb = newMasterPDB pdbName [ consSchema "toto" "toto" "Invest" ] "me" "comment1"
    repo.Put pdb |> ignore
    let repo2 = MasterPDBRepository(onRepoFailure, testFolder, pdbName) :> IMasterPDBRepository
    let pdb2 = repo2.Get()
...
let ``Log error when saving master PDB`` () =
    ...
    let repo = MasterPDBRepository((fun _ _ _ -> passedHere <- true), "*", pdbName) :> IMasterPDBRepository
    let pdb = newMasterPDB pdbName [ consSchema "toto" "toto" "Invest" ] "me" "comment1"
    let repo' = repo.Put pdb
