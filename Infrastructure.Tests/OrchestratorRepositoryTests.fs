...
let ``Save and load orchestrator`` () =
    let repo = OrchestratorRepository((fun _ -> raise), None, testFolder, orchestratorName) :> IOrchestratorRepository
...
let ``Log error when saving orchestrator`` () =
    ...
    let repo = OrchestratorRepository((fun _ _ -> passedHere <- true), None, "*", orchestratorName) :> IOrchestratorRepository
    let orchestrator = consOrchestrator [ "instance1"; "instance2"] "instance2"
    let repo' = repo.Put orchestrator
